	...
	int DFT(int idft, double* xr, double* xi, double* Xr_o, double* Xi_o, int N){
        ...
        #pragma omp parallel for shared(Xr_o, xr, xi, Xi_o, N, idft)
	  for (int k=0 ; k<N ; k++)
	    { 
        #pragma omp parallel
	      for (int n=0 ; n<N ; n++)  {
	    ...
	    if (idft==-1){
    #pragma omp parallel for shared(Xi_o, Xr_o, N)     
	    	for (int n=0 ; n<N ; n++){
	    	Xr_o[n] /=N;
	    	Xi_o[n] /=N;
	    }
...
#pragma omp parallel for shared(xi, xr, N)
	    for(int n=0; n < N;n++){
	...
	int setOutputZero(double* Xr_o, double* Xi_o, int N){
#pragma omp parallel for shared(Xr_o, Xi_o, N) 
	  for(int n=0; n < N;n++){
	       Xr_o[n] = 0.0;
	       Xi_o[n] = 0.0; 
	    }
...
#pragma omp parallel for shared(xr, xi, xr_check, xi_check, Xr_o, Xi_r, N)
	  for(int n=0; n < N;n++){
